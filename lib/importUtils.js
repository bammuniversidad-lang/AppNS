...
// Mapeo encabezado normalizado -> columna, para el archivo de Ventas que
// solo se usa para calcular la clasificación A/B/C/D (no se guarda el
// detalle, solo el resultado).
export const MAPEO_VENTAS_CLASIFICACION = {
  co: 'co',
  referencia: 'referencia',
  costopromediototal: 'costo_promedio_total',
  clientefactura: 'cliente_factura',
  sucursaldespacho: 'sucursal_despacho',
};

// Convierte filas crudas del archivo de Ventas a filas listas para
// calcular la clasificación (co, referencia, costo_promedio_total,
// cliente_factura, sucursal_despacho). No exige todos los campos: una
// fila sin cliente igual sirve para la clasificación por referencia, y
// viceversa.
export function mapearFilasVentasClasificacion(filasCrudas) {
  const filas = [];
  for (const filaCruda of filasCrudas) {
    const fila = {};
    for (const [encabezadoOriginal, valor] of Object.entries(filaCruda)) {
      const clave = MAPEO_VENTAS_CLASIFICACION[normalizarEncabezado(encabezadoOriginal)];
      if (clave) fila[clave] = valor;
    }
    if (!fila.co) continue; // fila vacía o de totales ("Gran total")

    fila.co = String(fila.co).trim();
    if (/^\d+$/.test(fila.co) && fila.co.length < 3) {
      fila.co = fila.co.padStart(3, '0');
    }
    fila.referencia = fila.referencia !== undefined && fila.referencia !== null ? String(fila.referencia).trim() : '';
    fila.cliente_factura = fila.cliente_factura !== undefined && fila.cliente_factura !== null ? String(fila.cliente_factura).trim() : '';
    fila.sucursal_despacho = fila.sucursal_despacho !== undefined && fila.sucursal_despacho !== null ? String(fila.sucursal_despacho).trim() : '';
    const num = Number(String(fila.costo_promedio_total ?? 0).replace(/[.,](?=\d{3}\b)/g, '').replace(',', '.'));
    fila.costo_promedio_total = isNaN(num) ? 0 : num;
...
export function mapearFilasPedidos(filasCrudas) {
  ...
  filasCrudas.forEach((filaCruda, indice) => {
    ...
    fila.co = String(fila.co).trim();
    // Si el C.O. venía en la celda de Excel como NÚMERO (en vez de texto),
    // Excel le quita los ceros a la izquierda (ej. "001" llega como "1").
    // Se rellenan de nuevo a 3 dígitos, para que no queden como un C.O.
    // distinto al que ya existía con los ceros.
    if (/^\d+$/.test(fila.co) && fila.co.length < 3) {
      fila.co = fila.co.padStart(3, '0');
    }
    ...
    // Estos dos son clave para que crucen bien con la clasificación por
    // Ventas (que se cruza por Cliente factura + Sucursal despacho): si
    // uno de los dos lados trae espacios y el otro no, nunca coinciden.
    if (fila.cliente_factura !== undefined && fila.cliente_factura !== null) {
      fila.cliente_factura = String(fila.cliente_factura).trim();
    }
    if (fila.sucursal_despacho !== undefined && fila.sucursal_despacho !== null) {
      fila.sucursal_despacho = String(fila.sucursal_despacho).trim();
    }
